...
class ReactionLvl3 : AppCompatActivity(), View.OnClickListener, Runnable {

    private lateinit var bindingReactionLvl3 : ReactionLvl3Binding
    ...
    private var flies : Int = 20
    private var fliesToHit : Int = 20
    private var caughtFlies : Int = 0
    private val highestAge : Long = 1500
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bindingReactionLvl3= ReactionLvl3Binding.inflate(layoutInflater)
        setContentView((bindingReactionLvl3.root))
        bindingReactionLvl3.iBPauseScreen.setOnClickListener{gameEngine.goToPause(this.javaClass)}

        scale = resources.displayMetrics.density
        gameboard = bindingReactionLvl3.FLGameboard
        lvl = getString(R.string.bt_lv3)
    ...
    private fun updateScreen(){
        val tvFliesLeft : TextView = bindingReactionLvl3.tVFlysLeft
        tvFliesLeft.text = fliesToHit.toString()

        val tvCaughtFlies : TextView = bindingReactionLvl3.tVCatchedFlies
        tvCaughtFlies.text = caughtFlies.toString()

        val tVTimeLeft : TextView = bindingReactionLvl3.tVTimeLeft
        tVTimeLeft.text = time.toString()

        val barFliesLeft : FrameLayout = bindingReactionLvl3.FLCatchedFlys
        val barTimeLeft : FrameLayout = bindingReactionLvl3.FLTimeLeft
    ...
    private fun countdownTIme(){
        ...
        if(!gameEngine.failedLevel(time, caughtFlies, lvl, ReactionLvl3::class.java)){
            if(!gameEngine.checkIfLevelPassed(caughtFlies)){
                handler.postDelayed(this, 1000)
            }else{
                gameEngine.levelPassed(lvl, "lvl_3_checked")
            }
        }
    }
