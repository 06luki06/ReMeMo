...
class Reaction_lvl4 : AppCompatActivity(), View.OnClickListener, Runnable {
    ...
    private var flys_to_hit : Int = 15
    ...
    private val HIGHEST_AGE : Long = 500
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        startGame()
        gameboard = bindingReaction_lvl4.FLGameboard
    ...
    private fun updateScreen(){
        val tV_flies_left : TextView = bindingReaction_lvl4.tVFlysLeft
        tV_flies_left.setText(flys_to_hit.toString())

        val tv_catched_flys : TextView = bindingReaction_lvl4.tVCatchedFlies
        tv_catched_flys.setText(catchedFlys.toString())

        val tVTimeLeft : TextView = bindingReaction_lvl4.tVTimeLeft
        tVTimeLeft.setText(time.toString())

        val flys_left_bar : FrameLayout = bindingReaction_lvl4.FLCatchedFlys
        val time_left_bar : FrameLayout = bindingReaction_lvl4.FLTimeLeft
        ...
        lpTime.width = Math.round(scale * time * 300 / 20)
    }

    private fun countdownTIme(){
        time--
        val randomNumber : Float = randdomNumberGenerator.nextFloat()
        val propability : Double = flys * 3.0 / 40
        if(propability > 1){
            showFlys()
    ...
    private fun gameOverScreen(){
        ...
        builder.setNeutralButton("Try Again"){dialog, which ->
            val intent : Intent = Intent(this, Reaction_lvl4::class.java)
            startIntent(intent)
        }.show()
    }
